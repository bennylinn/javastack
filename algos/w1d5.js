class SLL {
    ...
    reverse(current, prev){
        ...
        if(current!==null) {
            this.reverse(current.next, current);
            current.next = prev;
        } else {
            this.head = prev;
        }
    ...
    // do not use the length property
    nthToLast(n, current, prev){
        if(current===undefined) current = this.head;
        if(current===null) return null;
        if(n===0) {return prev} else {
            return this.nthToLast(n, current.next, current)
        }
    }
...
myFirstSLL.addToFront(new Node(10));
myFirstSLL.addToFront(new Node(13));
myFirstSLL.addToFront(new Node(16));
myFirstSLL.addToFront(new Node(21));
console.log(myFirstSLL);

myFirstSLL.reverse()
console.log(myFirstSLL);

console.log(myFirstSLL.nthToLast(2))